...
function showweatherDetails2(event) {
    ...
    const apiKey = '122171f72cf5d5ee5fe5de1fd3558b6f';
    const apiUrl2 = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}`;
        ...
        fetch(apiUrl2)
            ...
            .then(data => {
                // const currentTime = new Date().toLocaleTimeString()//Format current time
                // const currentDate = new Date().toLocaleDateString()
                const weatherInfo2 = document.getElementById('weatherInfo');
            ...
            .catch(error => {
                console.error('Error fetching weather:', error);
                const weatherInfo2 = document.getElementById('weatherInfo');
                weatherInfo2.innerHTML = `<p class='mt-5' style="background-color: rgb(255, 25, 25); color: white; border-radius: 8px; padding: 15px; width: 50vh"'>Failed to fetch weather. Please try again.</p>`;
                // weatherInfo.innerHTML = `<button class='btn btn-info btn-lg'>Failed to fetch weather. Please try again.</button>`;
            });
